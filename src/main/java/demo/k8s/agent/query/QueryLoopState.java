...
public record QueryLoopState(
    ...
    public static QueryLoopState initial() {
        return new QueryLoopState(0, 0, false, ContinuationReason.INITIAL);
    }

    public QueryLoopState nextTurn(ContinuationReason reason) {
        return new QueryLoopState(turnCount + 1, maxOutputTokensRecoveryCount, hasAttemptedReactiveCompact, reason);
    }

    public QueryLoopState withMaxOutputRecovery(int count) {
        return new QueryLoopState(turnCount, count, hasAttemptedReactiveCompact, lastContinuation);
    }

    public QueryLoopState withReactiveCompactAttempted() {
        return new QueryLoopState(turnCount, maxOutputTokensRecoveryCount, true, lastContinuation);
    }
}
